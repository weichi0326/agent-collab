...
- [ ] 先编写失败测试，覆盖安全 ID、正文键生成、字数、首次初始化、创建作品、创建章节、章节保存、8 MiB 限制和恢复记录重放。
- [ ] 运行 `npm.cmd test -- src/features/fictionist/repository.test.ts`，确认因模块不存在或接口缺失而失败。
- [ ] 在 `tauriStorage.ts` 暴露原始的 `getProjectStorageItem`、`setProjectStorageItem` 和 `removeProjectStorageItem`，原有 Zustand 适配器改为复用它们。
- [ ] 实现版本 1 索引和演示数据种子。卷中的 `chapterIds` 是唯一章序来源，正文键不写入实体。
- [ ] 实现恢复事务：先写 `multi-agent-fictionist-recovery`，再写正文、索引，最后删除恢复记录；加载时先重放未完成事务。
- [ ] 重新运行聚焦测试，确认全部通过。
- [ ] 提交：`功能：增加小说作品与章节仓储`
...
- [ ] 先编写失败测试，覆盖首次恢复、重启恢复选择、编辑变脏、保存成功、保存失败保持变脏、切章前自动保存和无章节作品。
- [ ] 运行 `npm.cmd test -- src/features/fictionist/fictionistStore.test.ts`，确认测试因 store 缺失而失败。
- [ ] 使用可注入仓储的 Zustand state creator 实现最小状态机；测试使用内存仓储，不模拟 store 自身。
- [ ] 在 `App.tsx` 启动阶段调用 `hydrate()`；小说数据加载失败不能阻止 master 工作台启动，但小说家页必须显示可重试错误。
- [ ] 重新运行聚焦测试，确认全部通过。
- [ ] 提交：`功能：增加小说编辑状态与重启恢复`
...
- [ ] 先扩展表征测试，要求页面不再出现“界面演示”和“关闭软件后不会保留”，并出现真实本地保存文案、加载失败重试文案和无章节空状态。
- [ ] 运行聚焦测试，确认新断言失败。
- [ ] 将书架统计改为从索引派生；新建作品成功后保留在书架；打开无章节作品时显示空状态和“新建章节”。
- [ ] 将编辑器正文和保存按钮接入 store；保存中禁用重复提交，失败显示错误并保持“有未保存修改”。
- [ ] 保留设定库、时间线、工作流和续写按钮为下一阶段演示，不更改其布局。
- [ ] 重新运行组件与领域测试，确认通过。
- [ ] 提交：`功能：接入真实小说作品与章节`
...
- [ ] 先编写失败测试，覆盖无守卫导航、守卫允许、守卫拒绝、守卫异常和注销守卫。
- [ ] 运行 `npm.cmd test -- src/settings/appNavigation.test.ts`，确认因模块缺失而失败。
- [ ] 实现单一当前守卫和异步导航门面，异常默认拒绝离开。
- [ ] 将所有用户可达的一级页面跳转改为经过 `requestAppView`。
- [ ] 小说家内部切换章节、作品、分区和返回书架同样先调用保存；保存失败保留原页面。
- [ ] 浏览器或 WebView 关闭时若仍有未保存正文，注册 `beforeunload` 防止静默退出。
- [ ] 运行导航、标题栏、引导和小说家相关测试，确认通过。
- [ ] 提交：`功能：保护小说未保存内容`
...
- [ ] 先在 Rust 测试中创建小说索引、章节、备份和临时文件，要求扫描能统计、清理能删除且不影响其他 `multi-agent-*` 数据。
- [ ] 运行 `cargo test storage::tests::cleanable_scan_includes_fictionist_data`，确认测试失败。
- [ ] 实现受控前缀扫描和删除，并把 `fictionist` 加入前端清理类型及测试。
- [ ] 确保 ErrorBoundary 的全项目数据重置也能清理小说动态键；桌面端通过受控前缀命令处理，浏览器端继续按 `multi-agent-` 清理。
- [ ] 更新《小说家与工作台集成规划》交接区：总体阶段、已完成、本轮改动、验证结果、已知限制、下一步。
- [ ] 运行 `cargo test` 和前端相关测试，确认通过。
- [ ] 提交：`功能：纳入小说数据清理与交接`
...
- [ ] 运行 `npm.cmd test`，要求全部 Vitest 测试通过。
- [ ] 运行 `npm.cmd run lint`，要求退出码 0。
- [ ] 运行 `npm.cmd run build`，要求生产构建成功；允许保留现有大包警告。
- [ ] 在 `app/src-tauri` 运行 `cargo test`，要求全部 Rust 测试通过。
- [ ] 运行 `cargo clippy -- -D warnings`，要求退出码 0。
- [ ] 运行 `git diff --check` 和 `git status --short`，确认只有本轮文件及原有未跟踪 `.superpowers/`。
- [ ] 检查《小说家与工作台集成规划》的交接状态与实际提交、测试结果一致。
