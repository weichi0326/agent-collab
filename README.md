...
小说家功能当前在 `fictionist` 分支开发。新电脑不要从本机复制 `data/`、`logs/`、`outputs/` 或 `python/venv`，应重新拉取源码并初始化环境：
...
随后运行 `环境配置器.bat`；已经具备 Node、Rust 和 Python 环境时，也可以按上面的手动方式启动。开始修改前先阅读 [docs/小说家开发交接.md](docs/小说家开发交接.md) 和 [docs/专业包开发规范.md](docs/专业包开发规范.md)。
...
- **工作流引擎**：依赖并行、失败隔离、门控 OR/AND/NOR、节点级重试、上游 JSON 优先流转，并支持 TXT、Markdown、Word、Excel 和思维导图产物。
- **专业任务底座**：专业包可以创建带不可变上下文的真实画布，任务来源会保留到保存画布、运行副本和运行历史；稳定结果角色用于把输出送回专业工作区确认。
- **小说家开发包**：`fictionist` 分支已实现书架、卷章、正文、分类与封面、大纲、设定库、时间线、全文搜索替换、专业 Agent、主备工作流和待确认写入。当前仍是内置开发形态，尚不能作为 `.mapp` 独立安装或卸载。
...
- [docs/专业包开发规范.md](docs/专业包开发规范.md) — master 与小说家、测试工程师等可执行专业包的长期开发边界
- [docs/小说家开发交接.md](docs/小说家开发交接.md) — `fictionist` 当前实现、目录、验证结果和下一步
- [小说家与工作台集成规划.md](小说家与工作台集成规划.md) — 小说家首个真实闭环、画布接入和专业包提取计划
